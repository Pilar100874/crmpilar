...
-- Recriar função com timeout aumentado e otimizações
...
AS $$
BEGIN
  -- Deletar mensagens das conversas (otimizado)
  DELETE FROM public.messages m
  USING public.conversations c
  WHERE m.conversation_id = c.id AND c.customer_id = p_customer_id;
  
  -- Deletar tags aplicadas nos chats
  DELETE FROM public.chat_tags_aplicadas t
  USING public.conversations c
  WHERE t.chat_id = c.id AND c.customer_id = p_customer_id;
  
  -- Deletar transferências de chat
  DELETE FROM public.chat_transferencias t
  USING public.conversations c
  WHERE t.chat_id = c.id AND c.customer_id = p_customer_id;
  ...
  -- Deletar itens de orçamentos
  DELETE FROM public.orcamento_itens oi
  USING public.orcamentos o
  WHERE oi.orcamento_id = o.id AND o.cliente_id = p_customer_id;
