...
BEGIN
  ...
  SELECT count(*) INTO n FROM public.pedidos_ecommerce WHERE customer_id = p_customer_id;
  IF n > 0 THEN v := v || jsonb_build_object('pedidos_ecommerce', n); END IF;
  ...
  SELECT count(*) INTO n FROM public.vendas_atribuidas WHERE customer_id = p_customer_id;
  IF n > 0 THEN v := v || jsonb_build_object('vendas_atribuidas', n); END IF;
...
GRANT EXECUTE ON FUNCTION public.check_customer_dependencies(uuid) TO authenticated;

-- Inactivate function
CREATE OR REPLACE FUNCTION public.inactivate_customer(p_customer_id uuid)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $$
BEGIN
  UPDATE public.customers SET ativo = false WHERE id = p_customer_id;
  RETURN FOUND;
END;
$$;

GRANT EXECUTE ON FUNCTION public.inactivate_customer(uuid) TO authenticated;

-- Fix delete cascade: correct column casts and column names, disable RLS inside
