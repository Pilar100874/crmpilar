...
serve(async (req) => {
  ...
  try {
    ...
    const clientId = config?.magalu_client_id;
    const redirectUri = config?.magalu_redirect_uri;
    ...
    // Magazine Luiza OAuth URL (placeholder - ajustar conforme documentação oficial)
    const authUrl = new URL("https://api.magazineluiza.com.br/oauth/authorize");
    authUrl.searchParams.set("client_id", clientId);
    authUrl.searchParams.set("redirect_uri", redirectUri);
    authUrl.searchParams.set("response_type", "code");
    authUrl.searchParams.set("state", state);

    console.log(`[Magalu] Auth URL gerada para conta ${contaMarketplaceId}`);
    ...
    console.error("[Magalu] Erro ao gerar URL de autorização:", error);
    return new Response(
