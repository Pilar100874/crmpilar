  ...
  try {
    ...
    const tokenResponse = await fetch("https://auth.olx.com.br/oauth/token", {
      ...
      body: new URLSearchParams({
        ...
        code,
        redirect_uri: config.olx_redirect_uri,
      }),
    });
    ...
    await supabase.from("marketplace_logs").insert({
      ...
      tipo: "oauth",
      mensagem: "Conta OLX conectada com sucesso",
      sucesso: true,
    });
    ...
    console.error("[OLX] Erro no callback:", error);
...
function generateHTML(success: boolean, message: string): string {
  return `<!DOCTYPE html>
<html>
<head><title>OLX - ${success ? "Sucesso" : "Erro"}</title>
...
<script>if(window.opener){window.opener.postMessage({type:'olx-oauth',success:${success},message:'${message}'},'*');}${success ? "setTimeout(()=>window.close(),2000);" : ""}</script>
</body></html>`;
}
