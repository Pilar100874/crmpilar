...
serve(async (req) => {
  try {
    ...
    console.log("[Magalu] Callback recebido:", { code: !!code, state });
    ...
    if (contaError || !contas || contas.length === 0) {
      console.error("[Magalu] Conta não encontrada para state:", state);
    ...
    // Trocar código por token (placeholder - ajustar conforme documentação oficial)
    const tokenResponse = await fetch("https://api.magazineluiza.com.br/oauth/token", {
      method: "POST",
    ...
    if (!tokenResponse.ok || tokenData.error) {
      console.error("[Magalu] Erro ao obter token:", tokenData);
    ...
    expiracao.setSeconds(expiracao.getSeconds() + (tokenData.expires_in || 3600));
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        ...
        seller_id: tokenData.seller_id || config.magalu_seller_id,
        status: "conectado",
    ...
    if (updateError) {
      console.error("[Magalu] Erro ao atualizar conta:", updateError);
    ...
    await supabase.from("marketplace_logs").insert({
      ...
      tipo: "conexao",
      mensagem: "Conta conectada com sucesso ao Magazine Luiza",
      sucesso: true,
    });

    console.log(`[Magalu] Conta ${conta.id} conectada com sucesso`);

    return new Response(generateHTML(true, "Conta Magazine Luiza conectada com sucesso!"), {
      headers: { "Content-Type": "text/html" },
    });
  } catch (err) {
    const error = err as Error;
    console.error("[Magalu] Erro no callback:", error);
...
function generateHTML(success: boolean, message: string): string {
  return `<!DOCTYPE html><html><head><title>Magazine Luiza - ${success ? "Sucesso" : "Erro"}</title><style>body{font-family:system-ui,sans-serif;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;background:#1a1a2e;color:white}.container{text-align:center;padding:40px}.icon{font-size:64px;margin-bottom:20px}.message{font-size:18px;margin-bottom:20px}.close-info{color:#888;font-size:14px}</style></head><body><div class="container"><div class="icon">${success ? "✅" : "❌"}</div><div class="message">${message}</div><div class="close-info">Esta janela será fechada automaticamente...</div></div><script>window.opener?.postMessage({type:'magalu-oauth',success:${success},message:'${message}'},'*');setTimeout(()=>window.close(),2000)</script></body></html>`;
}
