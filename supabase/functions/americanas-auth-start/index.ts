...
serve(async (req) => {
  ...
  try {
    ...
    const clientId = config?.americanas_client_id;
    const redirectUri = config?.americanas_redirect_uri;
    ...
    // Americanas OAuth URL
    const authUrl = new URL("https://api-marketplace.americanas.com/auth/authorize");
    authUrl.searchParams.set("client_id", clientId);
    authUrl.searchParams.set("redirect_uri", redirectUri);
    authUrl.searchParams.set("response_type", "code");
    authUrl.searchParams.set("state", `${contaMarketplaceId}:${state}`);

    console.log(`[Americanas] Auth URL gerada para conta ${contaMarketplaceId}`);
    ...
    console.error("[Americanas] Erro ao gerar URL de autorização:", error);
    return new Response(
