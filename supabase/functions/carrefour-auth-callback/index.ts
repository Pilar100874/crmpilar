...
serve(async (req) => {
  ...
  try {
    ...
    const tokenResponse = await fetch("https://api-marketplace.carrefour.com.br/oauth/token", {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: new URLSearchParams({
        grant_type: "authorization_code",
        client_id: config.carrefour_client_id,
        client_secret: config.carrefour_client_secret,
        code,
        redirect_uri: config.carrefour_redirect_uri,
      }),
    });
    ...
    await supabase
      ...
      .update({
        ...
        seller_id: tokenData.seller_id?.toString(),
    ...
    await supabase.from("marketplace_logs").insert({
      ...
      tipo: "oauth",
      mensagem: "Conta Carrefour conectada com sucesso",
      sucesso: true,
    });
    ...
    console.error("[Carrefour] Erro no callback:", error);
...
function generateHTML(success: boolean, message: string): string {
  return `<!DOCTYPE html>
<html>
<head><title>Carrefour - ${success ? "Sucesso" : "Erro"}</title>
...
<script>if(window.opener){window.opener.postMessage({type:'carrefour-oauth',success:${success},message:'${message}'},'*');}${success ? "setTimeout(()=>window.close(),2000);" : ""}</script>
</body></html>`;
}
