...
serve(async (req) => {
  ...
  try {
    ...
    const clientId = config?.via_client_id;
    const redirectUri = config?.via_redirect_uri;
    ...
    // Via Varejo (Casas Bahia) OAuth URL
    const authUrl = new URL("https://api.via.com.br/oauth/authorize");
    authUrl.searchParams.set("client_id", clientId);
    authUrl.searchParams.set("redirect_uri", redirectUri);
    authUrl.searchParams.set("response_type", "code");
    authUrl.searchParams.set("state", `${contaMarketplaceId}:${state}`);

    console.log(`[Casas Bahia] Auth URL gerada para conta ${contaMarketplaceId}`);
    ...
    console.error("[Casas Bahia] Erro:", error);
    return new Response(
