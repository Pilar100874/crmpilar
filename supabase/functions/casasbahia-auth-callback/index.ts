...
serve(async (req) => {
  ...
  try {
    ...
    const tokenResponse = await fetch("https://api.via.com.br/oauth/token", {
      method: "POST",
      ...
      body: new URLSearchParams({
        ...
        code,
        redirect_uri: config.via_redirect_uri,
      }),
    });
    ...
    await supabase.from("marketplace_logs").insert({
      ...
      tipo: "oauth",
      mensagem: "Conta Casas Bahia (Via) conectada com sucesso",
      sucesso: true,
    });
    ...
    console.error("[Casas Bahia] Erro no callback:", error);
...
function generateHTML(success: boolean, message: string): string {
  return `<!DOCTYPE html>
<html>
<head><title>Casas Bahia - ${success ? "Sucesso" : "Erro"}</title>
...
<script>if(window.opener){window.opener.postMessage({type:'casasbahia-oauth',success:${success},message:'${message}'},'*');}${success ? "setTimeout(()=>window.close(),2000);" : ""}</script>
</body></html>`;
}
