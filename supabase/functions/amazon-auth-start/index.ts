...
serve(async (req) => {
  ...
  try {
    ...
    const clientId = config?.amz_client_id;
    const redirectUri = config?.amz_redirect_uri;
    ...
    const authUrl = new URL("https://sellercentral.amazon.com.br/apps/authorize/consent");
    authUrl.searchParams.set("application_id", clientId);
    authUrl.searchParams.set("state", state);
    authUrl.searchParams.set("redirect_uri", redirectUri);

    console.log(`[Amazon] Auth URL gerada para conta ${contaMarketplaceId}`);
    ...
    console.error("[Amazon] Erro ao gerar URL de autorização:", error);
    return new Response(
