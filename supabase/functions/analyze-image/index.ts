...
serve(async (req) => {
  ...
  try {
    ...
    const tipoLabel = tipoObjeto || "objetos/volumes";

    const systemPrompt = `Você é um sistema de visão computacional de ALTÍSSIMA PRECISÃO, especializado em contar itens em imagens industriais e logísticas.

O USUÁRIO SOLICITOU CONTAR: "${tipoLabel}"

REGRA FUNDAMENTAL: Você deve contar EXCLUSIVAMENTE "${tipoLabel}". IGNORE COMPLETAMENTE qualquer outro tipo de material na imagem, incluindo mas não limitado a:
- Pallets e estrados de madeira
- Cintas, fitas de amarração, plástico filme (stretch)
- Papelão de proteção e embalagens externas
- Suportes, prateleiras, estruturas metálicas
- Qualquer material que NÃO seja "${tipoLabel}"

MÉTODO DE CONTAGEM OBRIGATÓRIO — siga rigorosamente:
1. PRIMEIRO, observe a imagem por completo e identifique APENAS os "${tipoLabel}" visíveis.
...
- Itens parcialmente visíveis nas bordas: conte se QUALQUER parte significativa é visível.
- Se houver dúvida se algo é 1 item ou 2, observe as linhas de separação entre eles.
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: systemPrompt },
          {
            role: "user",
            content: [
              { type: "text", text: `Analise esta imagem. Conte SOMENTE "${tipoLabel}" — ignore qualquer outro material (pallets, fitas, embalagens etc). Use o método de fileiras, verifique contando novamente, e retorne bounding boxes individuais.` },
