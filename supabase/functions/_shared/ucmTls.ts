/**
 * O UCM entrega só o certificado do site, sem a cadeia da Let's Encrypt.
 * Por isso confiamos aqui, explicitamente, no intermediário YR2 e na raiz Root YR.
 */
const CADEIA_LETS_ENCRYPT = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;

let clienteCache: unknown;

/** Cliente HTTP que aceita a cadeia da Let's Encrypt (quando o runtime permitir). */
export function clienteUcmTls(): { client?: unknown } {
  if (clienteCache !== undefined) return clienteCache ? { client: clienteCache } : {};
  try {
    const criar = (Deno as unknown as { createHttpClient?: (o: unknown) => unknown }).createHttpClient;
    clienteCache = criar ? criar({ caCerts: [CADEIA_LETS_ENCRYPT] }) : null;
  } catch (erro) {
    console.log("Sem cliente TLS personalizado:", erro instanceof Error ? erro.message : erro);
    clienteCache = null;
  }
  return clienteCache ? { client: clienteCache } : {};
}
