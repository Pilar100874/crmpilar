...
serve(async (req) => {
  ...
  try {
    ...
    const clientId = config?.olx_client_id;
    const redirectUri = config?.olx_redirect_uri;
    ...
    authUrl.searchParams.set("scope", "basic ads");
    authUrl.searchParams.set("state", `${contaMarketplaceId}:${state}`);

    console.log(`[OLX] Auth URL gerada para conta ${contaMarketplaceId}`);
