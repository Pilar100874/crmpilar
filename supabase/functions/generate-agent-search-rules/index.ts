...
serve(async (req) => {
  ...
  try {
    ...
    const { messages, regras_atuais } = await req.json();
    ...
    const systemPrompt = `Você é um especialista em criar REGRAS DE BUSCA E NEGÓCIO para agentes de IA que consultam catálogos de produtos.
