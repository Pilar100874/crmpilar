...
async function generateHeroFrame(params: any): Promise<string | null> {
  ...
  try {
    ...
    editContent.push({ type: "text", text: `TASK: Create a single realistic PHOTOMONTAGE image.\nYou MUST use the EXACT subjects from the photos above. The product is PRIORITY #1 and must be naturally integrated in the scene while keeping the reference packaging IDENTICAL: label, printed text, typography, colors, logo, cap/lid, material, shape and proportions. Do not redraw/reconstruct/reinterpret the package and do not paste the original photo as a flat overlay. If holding it would hide, bend, deform or rewrite the packaging, place the unchanged product in the person's open hand or directly on a REAL EXISTING textured surface already visible in the scene and have the person point to or present it. Do NOT create a new table, pedestal, white base, clean platform, card, frame, sticker, blank caption bar, grey pill, rounded rectangle, or any artificial support/background behind or under the product. IGNORE the white background of the reference photo — extract only the product and blend it into the scene with realistic shadow, perspective and lighting. IGNORE any red annotations, circles, arrows, paint marks, correction highlights, masks or review markings in the references: they are not part of the product/person/scene and must never appear in the output. The person's FACE must be IDENTICAL.\n\nScene: ${sceneDesc}\nAspect ratio: ${params.aspectRatio || '16:9'}` });
