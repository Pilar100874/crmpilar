  ...
  try {
    ...
    for (const entry of sortedEntries) {
      ...
      if (entry.role === 'PRODUCT - DO NOT MODIFY') {
        editContent.push({ type: "text", text: `↑ ⚠️ PRIORITY #1 — PRODUCT REFERENCE (design guide only). REGENERATE this product from scratch as a NEW photograph rendered by the model, fully integrated in the scene's lighting, perspective and camera angle. 🚫 ABSOLUTELY FORBIDDEN to copy, paste, splice, cut-out, sticker-overlay or reuse pixels from this reference photo on top of the generated image. 🚫 EXACTLY ONE instance of the product in the final image — no duplicate copy beside the person, no showcase copy, no inset, no badge, no side-by-side. The product in the output MUST be a freshly rendered version that shares the EXACT same packaging design (label, printed text, typography, logo, colors, cap/lid, material, shape, proportions) but is REPAINTED inside the scene as if photographed there. No flat sticker, no floating cut-out, no separate layer, no visible border, no mismatched lighting.` });
    ...
    editContent.push({ type: "text", text: `TASK: Create a single realistic PHOTOMONTAGE image where the product is REGENERATED inside the scene (not pasted on top).\n\n🚫 ABSOLUTELY FORBIDDEN: copying, pasting, splicing, sticker-overlaying, cutting out or reusing pixels from the product reference image on top of the output. No flat sticker. No floating cut-out. No visible border around the product. No separate layer with mismatched lighting/perspective.\n\n🚫 EXACTLY ONE INSTANCE OF THE PRODUCT IN THE FINAL IMAGE. Do NOT render a second copy, duplicate, mirrored copy, showcase copy, "product shot" beside the person, inset thumbnail, corner badge, side-by-side reference, before/after, or any additional floating instance of the product anywhere in the frame. The product appears ONE TIME ONLY, fully integrated where the person interacts with it.\n\n✅ REQUIRED: Render the product AGAIN from scratch, fully integrated in the scene's camera angle, lighting, perspective and shadows — as if photographed live in this scene. Keep its packaging design IDENTICAL to the reference (label, printed text, typography, colors, logo, cap/lid, material, shape, proportions), but REPAINT it inside the scene. The product is PRIORITY #1.\n\nIf holding it would hide, bend, deform or rewrite the packaging, place the unchanged product in the person's open hand or directly on a REAL EXISTING textured surface already visible in the scene and have the person point to or present it. Do NOT create a new table, pedestal, white base, clean platform, card, frame, sticker, blank caption bar, grey pill, rounded rectangle, or any artificial support/background behind or under the product. IGNORE the white background of the reference photo — extract only the product design and blend it into the scene with realistic shadow, perspective and lighting. IGNORE any red annotations, circles, arrows, paint marks, correction highlights, masks or review markings in the references: they are not part of the product/person/scene and must never appear in the output. The person's FACE must be IDENTICAL.\n\nScene: ${sceneDesc}\nAspect ratio: ${params.aspectRatio || '16:9'}` });

    const data = await callGateway(LOVABLE_API_KEY, {
      model: "google/gemini-3-pro-image-preview",
      messages: [
        { role: "system", content: "You are a professional photo compositor. Your ABSOLUTE #1 RULE is PRODUCT FIDELITY. The product must be naturally integrated in the scene while preserving the exact reference packaging: label, printed text, typography, colors, logo, proportions, material, cap/lid and shape. NEVER redesign, recolor, simplify, stylize, rotate into an invented view, paste as a flat sticker, or modify the product in any way. EXACTLY ONE instance of the product appears in the final image — never a second copy, duplicate, mirrored copy, showcase copy, inset thumbnail, corner badge, side-by-side reference, before/after, or floating extra instance. Hands, shadows, reflections, style references and brand identity are secondary and must not cover, deform or reinterpret the packaging. If holding the product would alter or hide the package, keep the product in the person's open hand or directly on a REAL EXISTING textured surface already visible in the scene and make the person point to or present it. NEVER put the product inside a white/colored square, card, frame, sticker, floating pedestal, white base, blank caption bar, grey pill, rounded rectangle, or artificial support. Ignore the white background of the reference image — extract only the product and blend it into the scene with realistic shadow, perspective and lighting. Ignore all red annotations, circles, arrows, paint marks, correction highlights, masks or review markings in reference images; they are external notes, not scene content, and must never be rendered. The PERSON/INFLUENCER face must remain identical to the reference. Product fidelity beats hand interaction and artistic style." },
...
REFERENCE IMAGE PRESERVATION: Any reference images provided (product, influencer, person, logo, clothing) MUST appear FULLY VISIBLE and UNCROPPED within the center strip. Scale them proportionally to fit within the ${safeZoneH}px tall strip.`;
        ...
        if (strictImages.length > 0) {
          ...
          const interactionBlock = hasProductAndInfluencer
            ...
            : '';
          const editPrompt = `TASK: Generate ONE final advertising photograph. Use the reference images ONLY as visual guides for appearance.\n\nSTRICT OUTPUT RULES:\n- The product must appear EXACTLY ONE time in the final scene.\n- Do NOT show the product reference photo itself anywhere in the image.\n- Do NOT create a second product beside the person, a comparison copy, a floating copy, a catalog cutout, a side-by-side reference, a thumbnail, a card, a label strip, a white box, or any extra isolated packaging.\n- Do NOT paste, overlay, splice, frame, or place the original product photo on top of the generated scene.\n- Render the final product as a natural part of the single photograph while preserving the package design: label, logo, printed text, colors, cap/lid, shape, material and proportions.\n\nPRIORITY ORDER:\n1. PRODUCT packaging fidelity.\n2. PERSON/INFLUENCER face identity.\n3. Background, style, lighting and identity visual.${interactionBlock}\n\nSubjects: ${subjectDescriptions}\n\nScene description:\n${params.prompt}`;
          editContent.push({ type: "text", text: editPrompt });

          const editSystemPrompt = `You generate a single seamless advertising photo from visual references. References are guides only; never render a reference image as a separate object in the output. ABSOLUTE RULE: the product appears ONCE and only once, integrated in the scene. No duplicate product, no side product, no product photo stuck beside the person, no inset, no card, no frame, no white square, no label strip, no flat overlay, no catalog cutout, no before/after, no comparison layout. Preserve the exact product packaging design while rendering it naturally in the scene. Ignore red annotations and ignore reference backgrounds. Person identity must match the reference. If holding would cause duplication or label damage, place the single product on an existing real surface or in an open hand, with the person presenting it.${dimensionInstruction}`;
