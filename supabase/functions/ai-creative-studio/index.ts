...
REFERENCE IMAGE PRESERVATION: Any reference images provided (product, influencer, person, logo, clothing) MUST appear FULLY VISIBLE and UNCROPPED within the center strip. Scale them proportionally to fit within the ${safeZoneH}px tall strip.`;
        ...
        if (strictImages.length > 0) {
          ...
          const editSystemPrompt = `You are a professional photo compositor specializing in premium brand advertising. ABSOLUTE PRODUCT LOCK: integrate the real product naturally into the scene while preserving the exact packaging design from the reference. Do not paste the original photo as a flat sticker and do not recreate a different package. Do not include the product reference background or any white box/card/base, grey caption bar, rounded rectangle, placeholder strip, pedestal, or artificial support. Ignore red annotations, circles, arrows, paint marks, correction highlights, masks, red boxes and any review markings in the reference images; they are external notes and must never appear in the final image. You may adjust perspective, scale and lighting only for realism; never change labels, logos, colors, printed text, cap/lid, shape, material or proportions. Fidelity > artistic quality. Priority #2: Person's face identical to reference. When both product and person are present, prefer presentation/pointing/touching-near the product instead of gripping over the front label. Use rule-of-thirds composition, soft professional lighting, and clean blurred background.${dimensionInstruction}`;
