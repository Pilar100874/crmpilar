...
REFERENCE IMAGE PRESERVATION: Any reference images provided (product, influencer, person, logo, clothing) MUST appear FULLY VISIBLE and UNCROPPED within the center strip. Scale them proportionally to fit within the ${safeZoneH}px tall strip.`;
        ...
        if (strictImages.length > 0) {
          ...
          const interactionBlock = hasProductAndInfluencer
            ? `\n\nCOMPOSITION & INTERACTION (MANDATORY when both product and person are present):\n- The person MUST be HOLDING the product in their hand — fingers naturally wrapped around the packaging.\n- The product must occupy 25-35% of the image area, positioned in the FOREGROUND or CENTER.\n- Use RULE OF THIRDS: product at one intersection point, person at another.\n- The product and person must be CLOSE TOGETHER forming a VISUAL UNIT — never separated.\n- The person's hand holding the product must look REALISTIC with correct finger anatomy.\n- Lighting: professional soft light highlighting the product without harsh shadows.\n- Background: clean, slightly blurred (bokeh) to make product and person stand out.\n- Style: premium brand advertising photography.\n- NEVER place the product on a table/surface away from the person.\n- NEVER generate the person without physically touching/holding the product.`
            : '';
