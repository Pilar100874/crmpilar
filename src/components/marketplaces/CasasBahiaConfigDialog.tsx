...
interface CasasBahiaConfigDialogProps {
  open: boolean;
  ...
  currentConfig?: {
    ...
    via_redirect_uri?: string;
  };
  redirectUri: string;
  onSaved?: () => void;
}

export function CasasBahiaConfigDialog({
  open,
  onOpenChange,
  contaId,
  contaNome,
  ...
  useEffect(() => {
    if (currentConfig) {
      setConfig({
        client_id: currentConfig.via_client_id || "",
        client_secret: currentConfig.via_client_secret || "",
      });
    }
  }, [currentConfig, open]);
  ...
  const handleSave = async () => {
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          configuracoes: {
            via_client_id: config.client_id,
            via_client_secret: config.client_secret,
            via_redirect_uri: redirectUri,
          },
        })
        .eq("id", contaId);
      ...
      if (contaData?.estabelecimento_id) {
        await supabase.from("marketplace_logs").insert({
          ...
          tipo: "config",
          mensagem: "Credenciais Casas Bahia (Via) configuradas",
          sucesso: true,
        });
      }
