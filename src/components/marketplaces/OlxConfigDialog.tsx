...
interface OlxConfigDialogProps {
  open: boolean;
  ...
  currentConfig?: {
    ...
    olx_redirect_uri?: string;
  };
  redirectUri: string;
  onSaved?: () => void;
}

export function OlxConfigDialog({
  open,
  onOpenChange,
  contaId,
  contaNome,
  ...
  useEffect(() => {
    if (currentConfig) {
      setConfig({
        client_id: currentConfig.olx_client_id || "",
        client_secret: currentConfig.olx_client_secret || "",
      });
    }
  }, [currentConfig, open]);
  ...
  const handleSave = async () => {
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          configuracoes: {
            olx_client_id: config.client_id,
            olx_client_secret: config.client_secret,
            olx_redirect_uri: redirectUri,
          },
        })
        .eq("id", contaId);
      ...
      if (contaData?.estabelecimento_id) {
        await supabase.from("marketplace_logs").insert({
          ...
          tipo: "config",
          mensagem: "Credenciais OLX configuradas",
          sucesso: true,
        });
      }
