...
interface AmericanasConfigDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  contaId: string;
  contaNome: string;
  currentConfig?: {
    americanas_client_id?: string;
    americanas_client_secret?: string;
    americanas_redirect_uri?: string;
  };
  redirectUri: string;
  onSaved?: () => void;
}

export function AmericanasConfigDialog({
  open,
  onOpenChange,
  contaId,
  contaNome,
  ...
  useEffect(() => {
    if (currentConfig) {
      setConfig({
        client_id: currentConfig.americanas_client_id || "",
        client_secret: currentConfig.americanas_client_secret || "",
      });
    }
  }, [currentConfig, open]);
  ...
  const handleSave = async () => {
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          configuracoes: {
            americanas_client_id: config.client_id,
            americanas_client_secret: config.client_secret,
            americanas_redirect_uri: redirectUri,
          },
        })
        .eq("id", contaId);
      ...
      if (contaData?.estabelecimento_id) {
        await supabase.from("marketplace_logs").insert({
          ...
          tipo: "config",
          mensagem: "Credenciais Americanas configuradas",
          sucesso: true,
        });
      }
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Configurar Americanas Marketplace
          </DialogTitle>
          <DialogDescription>
