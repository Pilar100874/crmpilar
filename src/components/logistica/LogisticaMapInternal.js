...
import { carregarDisposicao, obterChaveUsuario, salvarDisposicao, } from '@/lib/logistica/spiderfyPersistencia';
...
export const escalaRotuloPorZoom = (zoom) => {
...
const createVeiculoIcon = (status, compact = false, customColor, tipoVeiculo, ignicao, rotulo, tempoParado, enderecoParado, labelLado = 'right', labelDeslocY = 0, escala = 1) => {
...
const formatarTempoParado = (inicioIso) => {
...
const createParadaIcon = (cor, iconeName, compact = false) => {
...
const LogisticaMapInternal = ({ veiculos = [], routes = [], fullRouteBounds, paradasMarcadas = [], currentMarker, center = [-15.7801, -47.9292], zoom = 4, onVeiculoClick, onParadaClick, className = 'h-full w-full', fitBounds = true, fitBoundsPadding, compactIcons = false, disableInteraction = false, focusVeiculoId, focusTrigger, zoomMaximoSempre = false, modoFoco = false, focoZoom = 17, }) => {
    const mapContainerRef = useRef(null);
    const mapRef = useRef(null);
    const markersRef = useRef(new Map());
    const paradasMarkersRef = useRef(new Map());
    const routeLayersRef = useRef([]);
    const guiasRef = useRef(L.layerGroup());
    ...
    const chaveDisposicaoRef = useRef(null);
    const disposicaoRef = useRef({ grupos: {}, expandidos: [] });
    ...
    useEffect(() => {
        ...
        obterChaveUsuario().then(chave => {
            ...
            if (dados.expandidos.length)
                setGruposExpandidos(new Set(dados.expandidos));
    ...
    useEffect(() => {
        if (!disposicaoCarregada || !chaveDisposicaoRef.current)
            return;
    ...
    const [enderecosFallback, setEnderecosFallback] = useState({});
    const geocodePendenteRef = useRef(new Set());
    const ultimoBoundsRef = useRef(null);
    // Assinaturas para evitar recriar ícones/DOM a cada atualização (causa de "piscar")
    const iconSigRef = useRef(new Map());
    const paradaSigRef = useRef(new Map());
    const ultimoEnquadramentoRef = useRef('');
    // Estado de suavizacao do modo foco: ultima posicao conhecida, zoom e throttle
    const focoSeguirRef = useRef(null);
    ...
    const enquadrarTudo = useCallback((forcar = false) => {
        ...
        if (!map || !bounds || !fitBounds || autoPausadoRef.current)
            return;
        if (!forcar && !zoomMaximoSempre) {
            ...
            if (central.contains(bounds)) {
                ...
                const muitoLonge = spanBoundsLat / spanTelaLat < 0.25 && spanBoundsLng / spanTelaLng < 0.25;
                if (!muitoLonge)
                    return;
            }
        }
        if (!forcar) {
            ...
            if (sig === ultimoEnquadramentoRef.current)
                return;
    ...
    useEffect(() => {
        ...
        const map = mapRef.current;
        const aoInteragir = () => {
            if (movimentoProgramaticoRef.current)
                return;
            pausarAuto();
        };
    ...
    useEffect(() => {
        if (!mapRef.current || !fullRouteBounds || fullRouteBounds.length === 0)
            return;
        if (initialBoundsFittedRef.current)
            return;
        const bounds = L.latLngBounds(fullRouteBounds.map(c => [c.lat, c.lng]));
    ...
    useEffect(() => {
        ...
        const tempoPorVeiculo = new Map();
        const enderecoPorVeiculo = new Map();
        paradasMarcadas.forEach(p => {
            if (p.data_fim || p.ativa === false)
                return;
            if (p.mostrar_tempo) {
        ...
        const pontos = veiculosComPosicao
            .map(v => ({
            veiculo: v,
        ...
        if (!map.hasLayer(clusters))
            clusters.addTo(map);
        const criarClusterIcon = (qtd, expandido) => {
        ...
        grupos.forEach(grupo => {
            ...
            const centro = grupo.reduce((acc, p) => acc.add(p.ponto), L.point(0, 0)).divideBy(grupo.length);
            ...
            if (grupo.length >= LIMITE_CLUSTER && !expandido) {
                ...
                const clusterMarker = L.marker(centroLatLng, {
                    ...
                    zIndexOffset: 1000,
                    riseOnHover: true,
                }).bindTooltip(`<strong>${grupo.length} veículos aqui</strong><br/>${placas}<br/><em>Clique para expandir</em>`, { direction: 'top', opacity: 0.95 });
            ...
            grupo.forEach((item, i) => {
                ...
                const destino = L.point(centro.x + Math.cos(angulo) * raio, centro.y + Math.sin(angulo) * raio);
        ...
        veiculosComPosicao.forEach(veiculo => {
            ...
            const pos = info?.pos ?? [veiculo.ultima_posicao.lat, veiculo.ultima_posicao.lng];
            ...
            if (info?.deslocado) {
                guias.addLayer(L.polyline([[veiculo.ultima_posicao.lat, veiculo.ultima_posicao.lng], pos], { color: veiculo.cor || '#94a3b8', weight: 1.5, opacity: 0.7, dashArray: '3,4', interactive: false }));
            }
            ...
            const criarIcone = () => createVeiculoIcon(veiculo.status, compactIcons, veiculo.cor, veiculo.tipo_veiculo, veiculo.ultima_posicao?.ignicao, veiculo.placa, tempo, enderecoBalao, info?.lado ?? 'right', info?.dy ?? 0, escalaLabel);
            if (existingMarker) {
                const atual = existingMarker.getLatLng();
                const alvo = L.latLng(pos);
        ...
        const allPoints = veiculosComPosicao.map(v => [v.ultima_posicao.lat, v.ultima_posicao.lng]);
    ...
    useEffect(() => {
        const pendentes = paradasMarcadas.filter(p => p.mostrar_endereco && !p.endereco && !p.data_fim && p.ativa !== false);
        if (!pendentes.length)
            return;
        let cancelado = false;
        (async () => {
            ...
            for (const p of pendentes) {
                ...
                if (geocodePendenteRef.current.has(chave) || enderecosFallback[chave])
                    continue;
                ...
                if (cancelado)
                    return;
                if (texto)
                    setEnderecosFallback(prev => ({ ...prev, [chave]: texto }));
            }
        })();
        return () => {
            cancelado = true;
    ...
    useEffect(() => {
        if (!mapRef.current || !focusVeiculoId)
            return;
        ...
        pausarAuto();
        const pos = [veiculo.ultima_posicao.lat, veiculo.ultima_posicao.lng];
    ...
    useEffect(() => {
        if (!mapRef.current || !modoFoco || !focusVeiculoId)
            return;
        const map = mapRef.current;
        const veiculo = veiculos.find(v => v.id === focusVeiculoId && v.ultima_posicao);
        if (!veiculo)
        ...
        if (!deveSeguir || focoAnimandoRef.current)
            return;
    ...
    useEffect(() => {
        ...
        paradasVisiveis.forEach(parada => {
            const pos = [parada.lat, parada.lng];
    ...
    return (<div className="relative w-full h-full">
      ...
      {fitBounds && autoPausado && (<button type="button" onClick={retomarAuto} className="absolute top-3 left-1/2 -translate-x-1/2 z-[1000] flex items-center gap-2 rounded-full border border-border bg-background/90 px-3 py-1.5 text-xs font-semibold text-foreground shadow-lg backdrop-blur hover:bg-accent">
