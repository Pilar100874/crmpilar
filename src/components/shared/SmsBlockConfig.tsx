...
/**
 * UI compartilhada para o bloco "Enviar SMS" nos diversos workflow builders
 * (bot, omnichannel, ads, automação de vendas, logística, e-commerce rules).
 * Persiste em config: { phoneNumbers: string[], message: string, outputVariable: string }.
 */
export const SmsBlockConfig = ({ config, onChange }: Props) => {
  const phoneNumbers: string[] = Array.isArray(config?.phoneNumbers)
    ? config.phoneNumbers
    : config?.phoneNumber ? [config.phoneNumber] : [""];

  const updateNumbers = (next: string[]) => onChange("phoneNumbers", next);
    ...
    <div className="space-y-4">
      <Alert>
        ...
        <AlertDescription className="text-xs">
          Envia SMS para um ou mais números pelo gateway do estabelecimento.
          Variáveis (ex.: <code>{"{{telefone}}"}</code>) são substituídas no envio.
        </AlertDescription>
      </Alert>
      ...
      <div className="space-y-2">
        ...
        <Button type="button" variant="outline" size="sm" onClick={() => updateNumbers([...phoneNumbers, ""])}>
      ...
      <div className="space-y-2">
        ...
        <p className="text-[11px] text-muted-foreground">
          SMS comuns têm limite de 160 caracteres — textos maiores podem ser divididos pela operadora.
        </p>
      </div>
