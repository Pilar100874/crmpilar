...
// Organizar blocos por categoria com cores específicas
const blockCategories = [
  {
    name: "IA",
    icon: "Sparkles",
    iconColor: "#9333ea", // purple-600
    bgColor: "rgba(147, 51, 234, 0.1)", // purple with opacity
          ...
          {filteredCategories.map((category) => {
            ...
            return (
              <Collapsible
                ...
                className="group"
              >
                <CollapsibleTrigger className="flex items-center justify-between w-full px-2 py-1.5 rounded-lg hover:bg-muted transition-all duration-150 group border border-transparent hover:shadow-sm">
                  <div className="flex items-center gap-2">
                    {CategoryIcon && (
                      <div 
                      ...
                      >
                        <CategoryIcon className="w-3 h-3" style={{ color: category.iconColor }} />
                      </div>
                    )}
                    <span className="font-semibold text-xs text-foreground transition-colors">{category.name}</span>
                  </div>
                  <ChevronDown 
                    className={`w-3 h-3 transition-all duration-150 ${isOpen ? 'rotate-180' : ''}`}
                    style={{ color: isOpen ? category.iconColor : 'hsl(var(--muted-foreground))' }}
                  />
                </CollapsibleTrigger>
