          ...
          if (iw && ih) {
            ...
            if (isGrid) {
              ...
              if (carouselMode === 'independent') {
                ...
                formatNote = `Instagram Carousel ${slides} slides (${iw}x${ih}px total, each slide ${slideW}x${ih}px). Create ONE SINGLE CONTINUOUS WIDE image where EACH SLIDE is a COMPLETE, INDEPENDENTLY BEAUTIFUL composition. Each ${slideW}x${ih}px slice must work as a STANDALONE gorgeous image — with its own visual focus, proper subject framing, and balanced composition (like professional wedding photography where each photo is stunning alone). BUT when all ${slides} slides are placed side by side, they MUST form ONE unified panoramic scene with seamless flowing background, consistent lighting, and a cohesive visual narrative. The person/influencer and product should be positioned so they appear naturally within individual slides while contributing to the overall panorama. NO borders, NO frames, NO dividers.`;
              }
            }
