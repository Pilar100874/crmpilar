...
// ──────────────────────────────────────────────────────────────
// Reel Script ⇆ Strategy Engine importer
// Loads ONLY 'reel' artifacts and lets user pick one of the
// script variations (scripts[]) generated by the Roteirista de Reels.
// ──────────────────────────────────────────────────────────────
function ReelScriptStrategyImporter({ onImport }: { onImport: (scenes: any[], label: string) => void }) {
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [projects, setProjects] = useState<any[]>([]);
  const [artifacts, setArtifacts] = useState<any[]>([]);
  ...
  const mapScenes = (rawScenes: any[]): any[] =>
    (rawScenes || []).map((s: any) => ({
      description: s.descricao_visual || s.descricao || '',
      duration: snapDuration(toNum(s.duracao_segundos ?? s.duracao ?? 5)),
      narration: s.narracao_voz || s.narracao || '',
  ...
  const getScripts = (artifact: any): any[] => {
    ...
    if (!c) return [];
    if (Array.isArray(c.scripts) && c.scripts.length > 0) return c.scripts;
    // fallback: single roteiro at root
    if (Array.isArray(c.cenas_ai_video)) {
      return [{ titulo: c.titulo || c.hook || 'Roteiro', cenas_ai_video: c.cenas_ai_video, hook: c.hook, cta: c.cta }];
    }
    return [];
  };
  ...
  return (
    <div className="rounded-lg border border-rose-500/30 bg-rose-500/5 p-2.5 space-y-2">
      <p className="text-[11px] text-rose-300 leading-snug">
        📱 <strong>Importar roteiro do agente Roteirista de Reels</strong> — escolha uma das variações geradas no Motor de Estratégia.
      </p>
      {!open && (
        <Button size="sm" variant="outline" className="w-full h-7 text-[11px]" onClick={() => { setOpen(true); loadProjects(); }}>
          Selecionar roteiro de Reels
        </Button>
      )}
      {open && (
        <div className="space-y-2">
          ...
          {!loading && selectedProjectId && !selectedArtifact && (
            <>
              {artifacts.length === 0 ? (
                <p className="text-[10px] text-muted-foreground italic">Este projeto ainda não tem roteiros de Reels. Execute o agente Roteirista de Reels.</p>
              ) : (
                <div className="space-y-1">
                  {artifacts.map((a) => {
                    const scripts = getScripts(a);
                    return (
                      <button
                        key={a.id}
                        onClick={() => setSelectedArtifact(a)}
                        className="w-full text-left text-[11px] px-2 py-1.5 rounded-md bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/40"
                      >
                        📱 Reels — {scripts.length} variação(ões) · {new Date(a.created_at).toLocaleDateString('pt-BR')}
                      </button>
                    );
                  })}
                </div>
          ...
          {!loading && selectedArtifact && (
            <>
              <p className="text-[10px] text-muted-foreground">Escolha a variação de roteiro para usar:</p>
              <div className="space-y-1 max-h-60 overflow-y-auto pr-1">
                {getScripts(selectedArtifact).map((sc: any, idx: number) => {
                  ...
                  return (
                    <button
                      key={idx}
                      onClick={() => handlePickScript(selectedArtifact, idx)}
                      className="w-full text-left text-[11px] px-2 py-2 rounded-md bg-rose-500/10 hover:bg-rose-500/25 border border-rose-500/40 space-y-0.5"
                    >
              ...
              <Button size="sm" variant="ghost" className="h-6 text-[10px]" onClick={() => setSelectedArtifact(null)}>
                ← outras roteiros
              </Button>
            </>
          )}
