...
const StudioNodeComponent: React.FC<NodeProps> = memo(({ data, selected }) => {
  ...
  return (
    <div
      className={`
        relative rounded-xl border-2 bg-card shadow-md min-w-[200px] max-w-[280px]
        transition-all duration-200
        ${selected ? 'ring-2 ring-primary shadow-lg scale-[1.02]' : 'hover:shadow-lg'}
        ${nodeData.error ? 'border-destructive' : ''}
      `}
      style={{ borderColor: nodeData.error ? undefined : (selected ? color : `${color}60`) }}
    >
      {/* Header */}
      <div
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-muted-foreground">{meta?.description?.slice(0, 40)}</p>
        </div>
      ...
      <div className="px-3 py-2 space-y-1.5">
        ...
        {(nodeData.type === 'imageGen' || nodeData.type === 'imageEdit') && (
          <p className="text-[10px] text-muted-foreground">
            Modelo: {nodeData.config.model?.includes('pro') ? 'Nano Banana Pro' : 'Nano Banana'}
          </p>
        )}
        ...
        {nodeData.result && (
          <div className="mt-2 border-t pt-2">
            {typeof nodeData.result === 'string' && (
              <p className="text-xs line-clamp-4">{nodeData.result}</p>
            )}
            ...
            {nodeData.result?.text && !nodeData.result?.imageUrl && (
              <p className="text-xs line-clamp-3">{nodeData.result.text}</p>
            )}
          </div>
        )}
