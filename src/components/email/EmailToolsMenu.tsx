...
import { Plus, FileText, Languages, Sparkles, BookOpen, FileCheck } from "lucide-react";
...
function ToolbarBtn({ icon: Icon, title, onClick, disabled, active }: ToolbarBtnProps) {
  return (
    <TooltipProvider delayDuration={200}>
      <Tooltip>
        <TooltipTrigger asChild>
          <button
            className={active ? toolbarBtnActiveClass : toolbarBtnClass}
            onClick={onClick}
            disabled={disabled}
          >
            <Icon size={18} />
...
export function EmailToolsMenu({ estabelecimentoId, onInsertText, disabled }: EmailToolsMenuProps) {
  ...
  const handleImportReportSelect = async (reportId: string) => {
  ...
  const handleToolClick = (ferramenta: FerramentaConfig) => {
    ...
    switch (toolId) {
      ...
      case 'anexar_orcamento':
        // Implementar lógica de anexar orçamento
  ...
  ferramentasEmail.forEach((ferramenta, index) => {
        ...
        <TooltipProvider key={ferramenta.id} delayDuration={200}>
          <Tooltip>
            <Popover open={showTranslatePopover} onOpenChange={setShowTranslatePopover}>
              ...
              <PopoverContent className="w-56 p-3 rounded-xl shadow-xl border-border/50 bg-popover" align="start" sideOffset={8} style={{ zIndex: 9999 }}>
                <div className="space-y-3">
                  ...
                  <Button size="sm" onClick={() => { setShowTranslatePopover(false); setShowToolsMenu(false); }} className="w-full">
                    <Languages className="h-4 w-4 mr-2" /> Traduzir
                  </Button>
                </div>
              </PopoverContent>
            </Popover>
            <TooltipContent><p>{ferramenta.nome}</p></TooltipContent>
          </Tooltip>
        </TooltipProvider>
      );
        ...
        <TooltipProvider key={ferramenta.id} delayDuration={200}>
          <Tooltip>
            <Popover open={showImportReportsPopover} onOpenChange={setShowImportReportsPopover}>
              <TooltipTrigger asChild>
                <PopoverTrigger asChild>
                  <button className={showImportReportsPopover ? toolbarBtnActiveClass : toolbarBtnClass} disabled={disabled}>
                    <Icon size={18} />
                  </button>
                </PopoverTrigger>
              </TooltipTrigger>
              <PopoverContent className="w-72 p-3 rounded-xl shadow-xl border-border/50 bg-popover" align="start" sideOffset={8} style={{ zIndex: 9999 }}>
                <div className="space-y-3">
                  ...
                  {importReports.length > 0 ? (
                    <Select value={selectedImportReport || ""} onValueChange={handleImportReportSelect}>
