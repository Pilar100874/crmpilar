...
export default function TranspInspectionConfig() {
  ...
  const load = async () => {
    ...
    if (data) {
      ...
      const lista = base.length ? base : fallback;
      setPhotos(lista.length ? lista : normalize(TRANSP_ANGLES as any));
      setExitRequired((data as any).exit_photos_required ?? true);
      setEntryRequired((data as any).entry_photos_required ?? true);
    } else {
      setPhotos(normalize(TRANSP_ANGLES as any));
    }
    setLoading(false);
  };
  useEffect(() => { load(); }, []);
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Ângulos de foto usados na entrada e na saída — independente do Controle de Veículos.</p>
        </div>
      </div>
              ...
              {a.source === "ip_camera" && (
                <>
                  <div className="space-y-1">
                    ...
                    <Select value={a.camera_id ?? ""} onValueChange={(v) => updateAngle(i, { camera_id: v })}>
                      <SelectTrigger>
                  ...
                  <div className="space-y-1">
                    <Label className="text-xs" title="Use quando o veículo entra por um ponto e sai por outro — a captura da saída usará esta câmera.">
      ...
      <Card className="bg-muted/30">
        <CardContent className="p-4 text-sm text-muted-foreground space-y-1">
          <p className="flex items-center gap-2"><Badge variant="outline">Dica</Badge> A mesma lista de ângulos é usada tanto na entrada quanto na saída.</p>
