...
import { Plus, Pencil, Trash2, ExternalLink, MonitorPlay, PlayCircle } from "lucide-react";
...
export default function TvSignageDashboards() {
  ...
  useEffect(() => {
    ...
    supabase.from("unidades").select("id,nome").eq("ativo", true).order("nome").then(({ data }) => setGruposVeiculos(data || []));
