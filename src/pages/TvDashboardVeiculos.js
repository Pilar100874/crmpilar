...
import { Car, Clock, MapPin, WifiOff, Activity, RefreshCw, Timer, Zap, X, List, Pin, KeyRound, Power, ArrowLeft, Maximize2, Crosshair } from 'lucide-react';
...
export default function TvDashboardVeiculos() {
    ...
    const { progresso: progressoSaida } = useSaidaOculta(() => { try {
        window.close();
    }
    catch { } navigate(-1); });
    ...
    const [precosCombustivel, setPrecosCombustivel] = useState({ gasolina: 5.50, diesel: 5.80, etanol: 4.20 });
    const [kmRodadosHoje, setKmRodadosHoje] = useState({});
    const [focusVeiculoId, setFocusVeiculoId] = useState(null);
    ...
    const gruposFixos = useMemo(() => (searchParams.get('grupos') || '').split(',').map(s => s.trim()).filter(Boolean), [searchParams]);
        ...
        for (const veiculo of veiculosData) {
            ...
            if (posicoes && posicoes.length > 1) {
                ...
                for (let i = 1; i < posicoes.length; i++) {
                    const dist = calcularDistancia(posicoes[i - 1].lat, posicoes[i - 1].lng, posicoes[i].lat, posicoes[i].lng);
                    totalKm += dist;
                }
    ...
    const fetchVeiculos = useCallback(async () => {
        try {
            if (tvDeviceToken) {
                const data = await callTvDeviceFunction('tv-dashboard-veiculos', tvDeviceToken);
            ...
            if (veiculosError)
                throw veiculosError;
            const veiculosComStatus = await Promise.all((veiculosData || []).map(async (veiculo, index) => {
    ...
    const statusOrder = { movendo: 0, parado: 1, offline: 2 };
    const veiculosFiltrados = useMemo(() => {
        const base = gruposFixos.length
            ? veiculos.filter(v => {
                const gid = v.logistica_grupo_id || v.grupo_id;
    ...
    const veiculosParadosAlerta = useMemo(() => {
        return veiculos.filter(v => {
            if (v.status !== 'parado' || !v.ultima_posicao)
                return false;
    ...
    return (<>
      ...
      <div className={`fixed bg-black/55 backdrop-blur-sm rounded-xl border border-white/10 overflow-hidden flex flex-col transition-transform
        ...
        <div className="px-2 py-1.5 border-b border-white/10 flex items-center justify-between">
          ...
          {isMobile && (<Button variant="ghost" size="icon" onClick={() => setListaAberta(false)} className="h-6 w-6 text-white/70 hover:text-white">
              <X className="h-4 w-4"/>
            </Button>)}
        </div>
        ...
        <div className="px-2 py-1 border-b border-white/10 flex flex-wrap items-center gap-x-2 gap-y-0.5">
          {Object.keys(statusConfig).map((k) => (<span key={k} className="flex items-center gap-1 text-[9px] text-white/70 whitespace-nowrap">
              <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: statusConfig[k].hex }}/>
              {statusConfig[k].label} ({stats[k]})
            </span>))}
        </div>

        
        <div className="flex-1 overflow-y-auto">
          {veiculosFiltrados.length === 0 ? (<div className="p-3 text-center text-white/60 text-xs">
              Nenhum veículo
              ...
              {veiculosFiltrados.map((veiculo) => {
                ...
                return (<div key={veiculo.id} onClick={() => handleFocus(veiculo.id)} onDoubleClick={() => handleFocus(veiculo.id)} className={`px-2 py-1 hover:bg-white/5 transition-colors cursor-pointer ${focusVeiculoId === veiculo.id ? 'bg-white/10 ring-1 ring-primary' : ''}`}>
                    <div className="flex items-center justify-between gap-1.5">
                      <div className="flex items-center gap-1.5 min-w-0">
                        <div className="w-2.5 h-2.5 rounded-full border border-white/50 flex-shrink-0" style={{ backgroundColor: veiculo.cor }}/>
                        <span className="font-semibold text-[11px] whitespace-nowrap shrink-0" style={{ color: config.hex }} title={config.label}>
                          {veiculo.placa}
                        </span>
                      </div>
                      <button onClick={(e) => { e.stopPropagation(); togglePin(veiculo.id); }} title={isPinned ? 'Desafixar' : 'Fixar no mapa'} className={`p-0.5 rounded hover:bg-white/10 shrink-0 ${isPinned ? 'text-primary' : 'text-white/50'}`}>
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                      {veiculo.ultima_posicao && (<>
                          ...
                          <span title={typeof veiculo.ultima_posicao.ignicao === 'boolean'
                            ? veiculo.ultima_posicao.ignicao ? 'Ignição ligada' : 'Ignição desligada'
                            : 'Sem informação de ignição'} className={`inline-flex items-center gap-0.5 px-1 rounded ${veiculo.ultima_posicao.ignicao
                            ? 'bg-emerald-500/20 text-emerald-300'
                            : 'bg-white/10 text-white/50'}`}>
                            {veiculo.ultima_posicao.ignicao ? <KeyRound className="h-2.5 w-2.5"/> : <Power className="h-2.5 w-2.5"/>}
                          </span>
                          <CorteCombustivelBadge corte={veiculo.ultima_posicao.corte_combustivel} compact className="bg-white/10 text-white/80 text-[9px] px-1"/>
                        </>)}
                    </div>

                    {veiculo.motorista_atual?.nome && (<div className="mt-0.5 text-[9px] text-white/70 truncate" title={veiculo.motorista_atual.nome}>
      ...
      {isMobile && !listaAberta && (<Button onClick={() => setListaAberta(true)} className="fixed bottom-4 right-4 h-12 w-12 rounded-full shadow-xl bg-primary text-primary-foreground" style={{ zIndex: 999998 }} size="icon">
      ...
      <div className="fixed inset-0 bg-background overflow-hidden">
        ...
        <div className="absolute inset-0">
          {veiculosComPosicao.length === 0 ? (<div className="h-full w-full flex items-center justify-center bg-muted/50">
            ...
            </div>) : (<LazyLogisticaMap veiculos={veiculosComPosicao} paradasMarcadas={paradasMarcadas} className="absolute inset-0" fitBounds={!pinnedVeiculoId} fitBoundsPadding={fitBoundsPadding} zoomMaximoSempre={!pinnedVeiculoId} compactIcons focusVeiculoId={focusVeiculoId || undefined} focusTrigger={focusTrigger} modoFoco={modoFoco} focoZoom={modoFoco ? 18 : 17} onVeiculoClick={(v) => handleFocus(v.id)}/>)}
          {focusVeiculoId && (<div className="pointer-events-none absolute inset-0" style={{ zIndex: 999999 }}>
              <FocusLegend veiculo={veiculos.find(v => v.id === focusVeiculoId)} onClose={() => setFocusVeiculoId(null)}/>
            </div>)}
        </div>
        ...
        <div className="fixed top-3 left-3 flex items-center gap-2" style={{ zIndex: 999999 }}>
          {!modoTv && !tvDeviceToken && (<Button variant="secondary" size="icon" onClick={() => navigate(-1)} className="h-10 w-10 rounded-xl bg-background/95 backdrop-blur-md shadow-xl" data-tv-hide>
          ...
          {!modoTv && !tvDeviceToken && (<>
              {gruposFixos.length > 0 ? (<div className="px-4 py-2 bg-background/95 backdrop-blur-md rounded-xl shadow-xl text-sm font-medium" data-tv-hide>
                  {gruposFixos.length === 1
                    ? unidades.find(u => u.id === gruposFixos[0])?.nome || 'Grupo'
                    : `${gruposFixos.length} grupos`}
                </div>) : (<div className="bg-background/95 backdrop-blur-md rounded-xl shadow-xl" data-tv-hide>
                  <GrupoFilterSelect value={grupoId} onChange={setGrupoId} unidades={unidades} size="sm"/>
                </div>)}
              <Button variant={modoFoco ? 'default' : 'secondary'} size="sm" onClick={() => {
                setModoFoco(prev => {
                    const next = !prev;
                    if (next && focusVeiculoId)
                        setPinnedVeiculoId(focusVeiculoId);
                    if (!next)
                        setPinnedVeiculoId(null);
                    return next;
                });
            }} className="h-10 rounded-xl bg-background/95 backdrop-blur-md shadow-xl gap-1" title="Modo foco: centraliza e amplia no veículo selecionado" data-tv-hide>
                <Crosshair className="h-4 w-4"/>
                Modo Foco
              </Button>
              <Button variant="secondary" size="sm" onClick={showAll} className="h-10 rounded-xl bg-background/95 backdrop-blur-md shadow-xl gap-1" title="Mostrar todos os veículos no mapa" data-tv-hide>
        ...
        <div className="fixed bottom-3 left-3 right-20 md:right-auto space-y-2 md:max-w-[50%]" style={{ zIndex: 999999 }}>
          ...
          {veiculos.some(v => v.ultima_posicao && v.ultima_posicao.velocidade > 100) && (<div className="flex items-center gap-3 px-4 py-3 bg-red-500/20 backdrop-blur-md rounded-xl shadow-xl border border-red-500/30">
              <Zap className="h-5 w-5 text-red-500 animate-pulse flex-shrink-0"/>
              <span className="text-sm text-red-500 font-medium truncate">
                Velocidade: {veiculos.filter(v => v.ultima_posicao && v.ultima_posicao.velocidade > 100).map(v => `${v.placa} (${Math.round(v.ultima_posicao.velocidade)}km/h)`).join(', ')}
              </span>
            </div>)}

          {/* Alerta de Veículos Parados */}
          {veiculosParadosAlerta.length > 0 && (<div className="flex items-center gap-3 px-4 py-3 bg-amber-500/20 backdrop-blur-md rounded-xl shadow-xl border border-amber-500/30">
