...
export class MagaluService extends BaseMarketplaceService {
  ...
  async conectarConta(contaMarketplaceId: string): Promise<void> {
    ...
    const response = await fetch(`${SUPABASE_URL}/functions/v1/magalu-auth-start?contaMarketplaceId=${contaMarketplaceId}`);
    ...
    const authWindow = window.open(data.authUrl, "magalu-auth", "width=600,height=700,scrollbars=yes");
    ...
    return new Promise((resolve, reject) => {
      ...
      const messageHandler = (event: MessageEvent) => {
        if (event.data?.type === "magalu-oauth") {
  ...
  async sincronizarProdutos(contaMarketplaceId: string): Promise<void> {
    const { data, error } = await supabase.functions.invoke("magalu-sync", {
