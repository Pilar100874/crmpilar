...
export class OlxService extends BaseMarketplaceService {
  constructor() {
    super("OLX");
  }
  ...
  async conectarConta(contaMarketplaceId: string): Promise<void> {
    ...
    const response = await fetch(
      `${SUPABASE_URL}/functions/v1/olx-auth-start?contaMarketplaceId=${contaMarketplaceId}`
    );
  ...
  async sincronizarProdutos(contaMarketplaceId: string): Promise<void> {
    const { data, error } = await supabase.functions.invoke("olx-sync", {
      body: { contaMarketplaceId, action: "anuncios" },
    });
  ...
  async sincronizarPedidos(contaMarketplaceId: string): Promise<void> {
    const { data, error } = await supabase.functions.invoke("olx-sync", {
      body: { contaMarketplaceId, action: "mensagens" },
    });
