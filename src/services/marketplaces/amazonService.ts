...
export class AmazonService extends BaseMarketplaceService {
  ...
  async conectarConta(contaMarketplaceId: string): Promise<void> {
    ...
    const response = await fetch(`${SUPABASE_URL}/functions/v1/amazon-auth-start?contaMarketplaceId=${contaMarketplaceId}`);
    ...
    const authWindow = window.open(data.authUrl, "amazon-auth", "width=600,height=700,scrollbars=yes");
    ...
    return new Promise((resolve, reject) => {
      ...
      const messageHandler = (event: MessageEvent) => {
        if (event.data?.type === "amazon-oauth") {
