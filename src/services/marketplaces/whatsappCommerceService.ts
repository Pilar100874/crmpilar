...
export class WhatsAppCommerceService extends BaseMarketplaceService {
  constructor() {
    super("WhatsApp Commerce");
  }
  ...
  async conectarConta(contaMarketplaceId: string): Promise<void> {
    ...
    const response = await fetch(
      `${SUPABASE_URL}/functions/v1/whatsapp-commerce-auth-start?contaMarketplaceId=${contaMarketplaceId}`
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      const messageHandler = (event: MessageEvent) => {
        if (event.data?.type === "whatsapp-commerce-oauth") {
  ...
  async sincronizarProdutos(contaMarketplaceId: string): Promise<void> {
    const { data, error } = await supabase.functions.invoke("whatsapp-commerce-sync", {
      body: { contaMarketplaceId, action: "catalogo" },
    });
...
export const whatsappCommerceService = new WhatsAppCommerceService();
