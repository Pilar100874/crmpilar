...
export class CarrefourService extends BaseMarketplaceService {
  constructor() {
    super("Carrefour");
  }
  ...
  async conectarConta(contaMarketplaceId: string): Promise<void> {
    ...
    const response = await fetch(
      `${SUPABASE_URL}/functions/v1/carrefour-auth-start?contaMarketplaceId=${contaMarketplaceId}`
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      const messageHandler = (event: MessageEvent) => {
        if (event.data?.type === "carrefour-oauth") {
  ...
  async sincronizarProdutos(contaMarketplaceId: string): Promise<void> {
    const { data, error } = await supabase.functions.invoke("carrefour-sync", {
