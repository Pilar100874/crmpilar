...
export class CasasBahiaService extends BaseMarketplaceService {
  constructor() {
    super("Casas Bahia");
  }
  ...
  async conectarConta(contaMarketplaceId: string): Promise<void> {
    ...
    const response = await fetch(
      `${SUPABASE_URL}/functions/v1/casasbahia-auth-start?contaMarketplaceId=${contaMarketplaceId}`
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      const messageHandler = (event: MessageEvent) => {
        if (event.data?.type === "casasbahia-oauth") {
  ...
  async sincronizarProdutos(contaMarketplaceId: string): Promise<void> {
    const { data, error } = await supabase.functions.invoke("casasbahia-sync", {
...
export const casasBahiaService = new CasasBahiaService();
